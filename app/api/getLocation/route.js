...
export async function GET(request) {
  ...
  const API_ENDPOINT = `https://api.mapbox.com/geocoding/v5/mapbox.places/${longitude},${latitude}.json?access_token=${process.env.NEXT_PUBLIC_MAPBOX_BACKEND}&types=place&language=en&limit=1`;
  const res = await fetch(API_ENDPOINT);
  const data = await res.json();
